...
def generateJWT(channelId, iterative=True):
    url = f"{API_BASE_URL}auth-service/v1/oauth/token-service/token"
    ...
    if x.status_code == 200:
        ...
        if msg == 'OAuth Token Generated Successfully':
            ...
            s = f"{msg} for channelId: {str(channelId)}"
            if iterative:
                print(s)

            token = x.json()['data']['token']
            tokenMsg = f"Token:{token}"
            if iterative:
                print(tokenMsg)
            return token
        else:
...
def getEpidList(channelId):
    ...
    return [
        {"epid": "Subscription", "bid": entitlement}
...
def getHeaders():
    ...
    return {
        ...
        'device_details': '{"pl":"web","os":"WINDOWS","lo":"en-us","app":"1.36.35","dn":"PC","bv":103,"bn":"CHROME","device_id":"YVJNVFZWVlZ7S01UZmRZTWNNQ3lHe0RvS0VYS0NHSwA","device_type":"WEB","device_platform":"PC","device_category":"open","manufacturer":"WINDOWS_CHROME_103","model":"PC","sname":"%s"}'
        % sName,
        'kp': 'false',
        'locale': 'ENG',
